...
SELECT COUNT( DISTINCT name ) , membercost
FROM Facilities
WHERE membercost =0
...
the member name. */
SELECT DISTINCT (
CONCAT( f.name, ',', m.firstname, ' ', m.surname )
)
FROM Bookings b
...
SELECT f.name,
	   CONCAT(m.firstname, ' ', m.surname) AS User,
...
SELECT f.name,
	   sub.TotalRev
FROM Facilities f
JOIN (SELECT f.name AS name,
      SUM(CASE WHEN b.memid=0 THEN f.guestcost * b.slots
           ELSE f.membercost * b.slots END) AS TotalRev
      FROM Bookings b
      JOIN Facilities f ON b.facid = f.facid
      GROUP BY 1
      ) sub ON f.name = sub.name
WHERE TotalRev < 1000
GROUP BY 1
ORDER BY 2